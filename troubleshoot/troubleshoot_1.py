...
cls_model_path = os.path.join(project_root, "weights/classification-old.pth")
...
def classify_crop(image, classifier, device, threshold_sc=0.74):
    image_tensor = inference_transforms(image).unsqueeze(0).to(device)
    with torch.no_grad():
        output = classifier(image_tensor)
        probs = torch.nn.functional.softmax(output, dim=1)
    # SC class is assumed to be at index 0 based on figure_2C_step1.py
        ...
        for img_path in tqdm(image_files, desc=f"Patient {patient_id}", leave=False):
            try:
                ...
                for box in boxes:
                    ...
                    # Draw bounding boxes (Red for SC, Green for SN)
                    color = "red" if label == "SC" else "green"
                    draw.rectangle(((box[0], box[1]), (box[2], box[3])), outline=color, width=3)
                
                # Save the processed image as JPEG
                out_filename = os.path.splitext(os.path.basename(img_path))[0] + ".jpg"
                out_path = os.path.join(patient_out_dir, out_filename)
