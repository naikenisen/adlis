...
# Identify VP using class names when available
if num_classes != 2:
...
# Keep only correct predictions above confidence threshold
VP = VP[VP['pred_confidence'] >= CONFIDENCE_THRESHOLD]

class ImageSelectorApp:
    def __init__(self, root, vp_df, images_per_page=20, ncols=4):
        self.root = root
        self.root.title("Selection VP - ADLIS")
        self.vp_df = vp_df.sample(frac=1, random_state=42).reset_index(drop=True)
    ...
    def _build_ui(self):
        header = (
            f"Vrais positifs disponibles: {len(self.vp_df)} | "
        ...
        self.grid_frame.pack(fill=tk.BOTH, expand=True, padx=12, pady=6)
    ...
    def _render_page(self):
        ...
        total_images = len(self.vp_df)
        if total_images == 0:
            self.page_info.config(text="Aucune image VP trouvee.")
            return

        start = self.current_page * self.images_per_page
        end = min(start + self.images_per_page, total_images)
        page_df = self.vp_df.iloc[start:end]
    ...
    def next_page(self):
        total_pages = int(np.ceil(len(self.vp_df) / self.images_per_page))
    ...
    def save_selection(self):
        ...
        selected_df = self.vp_df[self.vp_df['image_path'].isin(self.selected_paths)].copy()
...
if __name__ == "__main__":
    if VP.empty:
        print("Aucune image VP ne passe le seuil de confiance.")
    else:
        app_root = tk.Tk()
        app = ImageSelectorApp(app_root, VP, images_per_page=IMAGES_PER_BOARD)
        app_root.mainloop()
